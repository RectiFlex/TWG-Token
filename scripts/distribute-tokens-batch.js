...
async function main() {
  try {
    console.log("===== BATCH DISTRIBUTING TWG TOKENS TO ZERO BALANCE ADDRESSES =====");
    ...
    // Define batch size (number of addresses to process in each batch)
    const BATCH_SIZE = 5;
    
    // Define starting batch index (set this to continue from a specific batch)
    const START_BATCH = 0;
    
    // Define the addresses to check (first few addresses with zero balance from list-zero-balance-addresses.js)
    ...
    console.log(`\nTotal addresses to check: ${addressesToCheck.length}`);
    ...
    for (const recipient of addressesToCheck) {
      ...
      if (currentBalance.isZero()) {
        ...
        console.log(`${recipient.address} (${recipient.category}): Zero balance - Will distribute ${recipient.amount} ${symbol}`);
      } else {
        console.log(`${recipient.address} (${recipient.category}): Has ${ethers.utils.formatUnits(currentBalance, decimals)} ${symbol} - Skipping`);
      }
    }
    
    console.log(`\nAddresses with zero balance: ${addressesToActuallyDistribute.length} out of ${addressesToCheck.length}`);
    ...
    const totalBatches = Math.ceil(addressesToActuallyDistribute.length / BATCH_SIZE);
    console.log(`\nProcessing in ${totalBatches} batches of ${BATCH_SIZE} addresses each.`);
    console.log(`Starting from batch ${START_BATCH + 1}`);
    
    // Process each batch
    for (let batchIndex = START_BATCH; batchIndex < totalBatches; batchIndex++) {
      const start = batchIndex * BATCH_SIZE;
      const end = Math.min((batchIndex + 1) * BATCH_SIZE, addressesToActuallyDistribute.length);
      const currentBatch = addressesToActuallyDistribute.slice(start, end);
      
      console.log(`\n=== PROCESSING BATCH ${batchIndex + 1}/${totalBatches} (Addresses ${start + 1}-${end}) ===`);
      ...
      for (const recipient of currentBatch) {
        ...
        try {
          // Execute the transfer with explicit gas limit
          ...
          // Continue with next address even if there's an error
        }
      }
      
      // Check remaining contract balance after each batch
      const currentContractBalance = await token.balanceOf(contractAddress);
      console.log(`\nRemaining contract balance: ${ethers.utils.formatUnits(currentContractBalance, decimals)} ${symbol}`);
      
      console.log(`\nBatch ${batchIndex + 1} complete. To continue with the next batch, set START_BATCH = ${batchIndex + 1}`);
      
      // For safety, let's only process one batch at a time
      break;
    }
