...
async function main() {
  try {
    console.log("===== CHECKING ZERO BALANCE ADDRESSES =====");
    ...
    // Define the addresses to check (same as in distribute-tokens.js)
    ...
    const addressesWithZeroBalance = [];
    const addressesWithNonZeroBalance = [];
    
    for (const recipient of addressesToCheck) {
      const currentBalance = await token.balanceOf(recipient.address);
      if (currentBalance.isZero()) {
        addressesWithZeroBalance.push(recipient);
      } else {
        addressesWithNonZeroBalance.push({
          ...recipient,
          balance: ethers.utils.formatUnits(currentBalance, decimals)
        });
      }
    }
    
    // Report on addresses with nonzero balance
    console.log(`\n=== ADDRESSES WITH TOKENS (${addressesWithNonZeroBalance.length}) ===`);
    for (const recipient of addressesWithNonZeroBalance) {
      console.log(`${recipient.address} (${recipient.category}): ${recipient.balance} ${symbol}`);
    }
    
    // Report on addresses with zero balance
    console.log(`\n=== ADDRESSES WITH ZERO BALANCE (${addressesWithZeroBalance.length}) ===`);
    if (addressesWithZeroBalance.length === 0) {
      console.log("No addresses with zero balance found.");
    } else {
      console.log("The following addresses have zero balance and need tokens:");
      
      let totalToDistribute = ethers.BigNumber.from("0");
      for (const recipient of addressesWithZeroBalance) {
        const amount = ethers.utils.parseUnits(recipient.amount, decimals);
        totalToDistribute = totalToDistribute.add(amount);
        console.log(`${recipient.address} (${recipient.category}): ${recipient.amount} ${symbol}`);
      }
      
      console.log(`\nTotal tokens needed: ${ethers.utils.formatUnits(totalToDistribute, decimals)} ${symbol}`);
      
      if (contractBalance.gte(totalToDistribute)) {
        console.log(`\n✅ Contract has enough tokens to distribute (${ethers.utils.formatUnits(contractBalance, decimals)} ${symbol})`);
        console.log(`\nTo distribute tokens to these addresses, run the following command:`);
        ...
        console.log(`Contract balance: ${ethers.utils.formatUnits(contractBalance, decimals)} ${symbol}`);
        console.log(`Required: ${ethers.utils.formatUnits(totalToDistribute, decimals)} ${symbol}`);
        console.log(`Shortfall: ${ethers.utils.formatUnits(totalToDistribute.sub(contractBalance), decimals)} ${symbol}`);
      }
    }
